...
if(respon.data.success){
  return {status: true, author: "iwan", result:{title:respon.data.data.title, link:`${baseUrl}/download?type=yt&link=${encodeURIComponent(respon.data.data.server_path.replace(/&/g, '%26').replace(/#/g, '%23').replace(/\+/g, '%2B'))}`}}
}else{
await delay(3)
...
return {status: true, author: "iwan", result:{title:get_video_data.data.title, link:`${baseUrl}/download?type=yt&link=${encodeURIComponent(download_video_ucep.data.download_link.replace(/&/g, '%26').replace(/#/g, '%23').replace(/\+/g, '%2B'))}`}}
}
}
}
}else{
